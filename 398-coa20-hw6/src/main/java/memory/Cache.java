...
public class Cache {	//
	...
	public int fetch(String sAddr, int len) {
		//TODO
		setStrategy(new SetAssociativeMapping(),new FIFOReplacement(),new WriteBackStrategy());
		int blockNo = getBlockNO(sAddr);
		int line = mappingStrategy.map(blockNo);
		if (line==-1){
			line = mappingStrategy.writeCache(blockNo);
		}
		return line;
	}
